...
def delete_resume(resume_id):
    ...
    file_path = os.path.join(RESUME_DIR, resume_id)
    if os.path.exists(file_path):
        os.remove(file_path)
        st.success(f"File '{resume_id}' deleted from disk")
    else:
        st.warning(f"File '{resume_id}' not found on disk")
...
def shortlist_resumes(job_description, top_k):
    results = vector_db.similarity_search(job_description, k=top_k)
    return results
...
st.set_page_config(page_title="AI Resume Shortlisting (Local RAG)")
st.title("📄 AI Resume Shortlisting System")
st.write("Local LLM + Local Embeddings | Normal RAG | Ranked Results")
